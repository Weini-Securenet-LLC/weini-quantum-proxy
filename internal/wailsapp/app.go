...
// CopyShareText copies shareable proxy URI text (e.g. ss/vmess/vless link) to the OS clipboard.
func (a *App) CopyShareText(text string) error {
	text = strings.TrimSpace(text)
	if text == "" {
		return fmt.Errorf("没有可复制的链接")
	}
	ctx := a.ctx
	if ctx == nil {
